#################################################
# HelloID-Conn-Prov-Target-Xedule-employees-Update
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        'UpdateAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Updating Xedule-employees account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatUpdateParams
            } else {
                Write-Information "[DryRun] Update Xedule-employees account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
        ...
        'NoChanges' {
            Write-Information "No changes to Xedule-employees account with accountReference: [$($actionContext.References.Account)]"
        ...
        'NotFound' {
            ...
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Xedule-employees account with accountReference: [$($actionContext.References.Account)] could not be found, possibly indicating that it could be deleted"
                    IsError = $true
                })
            break
        }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not update Xedule-employees account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not update Xedule-employees account. Error: $($ex.Exception.Message)"
