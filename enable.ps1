#################################################
# HelloID-Conn-Prov-Target-Xedule-employees-Enable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        'EnableAccount' {
            $body = @{
                ...
                InDienst  = @(
                    @{
                        ...
                        To   = (Get-Date).AddDays(9999).ToString('yyyy-MM-ddTHH:mm:ss')
                    }
                )
            }
            $splatUpdateParams = @{
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Enabling Xedule-employees account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatUpdateParams
            } else {
                Write-Information "[DryRun] Enable Xedule-employees account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable Xedule-employees account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not enable Xedule-employees account. Error: $($_.Exception.Message)"
