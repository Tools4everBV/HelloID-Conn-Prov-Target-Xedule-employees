##################################################
# HelloID-Conn-Prov-Target-Xedule-employees-Delete
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting Xedule-employees account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatDeleteParams

            } else {
                Write-Information "[DryRun] Delete Xedule-employees account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete Xedule-employees account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not delete Xedule-employees account. Error: $($_.Exception.Message)"
